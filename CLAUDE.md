...
## Astraeus Σ-9000 Agentic Environment

### Overview
This project now includes an advanced AI development environment powered by Astraeus Σ-9000, featuring specialized sub-agents for comprehensive development support. Each sub-agent is a hyper-specialized expert designed to provide analysis, guidance, and proposals without directly modifying source files.

### Core Philosophy
- **Declarative & Deterministic**: Sub-agents operate from explicit role definitions ensuring reproducible, context-aware results
- **Deep Specialization**: Each agent embodies 10+ years of expertise in their specific domain
- **Non-Destructive Analysis**: Sub-agents provide expert analysis and proposals, never directly modify code
- **Parallel Processing**: Multiple agents can work simultaneously without conflicts
- **Context Sharing**: Agents utilize MCP memory servers and shared documentation for knowledge continuity

### Interacting with Sub-Agents
Sub-agents are automatically invoked based on trigger conditions in their descriptions. The main orchestrator determines when to deploy specific expertise based on:
- Keywords in user requests
- Project context and current tasks  
- Identified needs for specialized analysis

### Memory Management
The `.claude/sub_agents_memory.md` file serves as a persistent knowledge base where sub-agents document:
- Important discoveries and decisions
- Cross-agent coordination information
- Project-specific insights and gotchas
- Failed approaches to avoid repetition

### Available Sub-Agent Expertise

The following 12 specialized sub-agents are now available for deployment:

#### 🔧 Core Technical Specialists
- **audio-processing-expert**: Real-time audio systems, signal processing, noise reduction, cross-platform audio
- **ml-transcription-specialist**: ASR optimization, Whisper models, OpenAI API, multi-language transcription  
- **realtime-systems-architect**: Low-latency systems, memory optimization, concurrent processing
- **python-async-expert**: AsyncIO, FastAPI, event loop optimization, async patterns
- **security-privacy-auditor**: Command injection prevention, audio data protection, OWASP security

#### ⚙️ Configuration & DevOps
- **config-deployment-specialist**: YAML configuration, cross-platform deployment, dependency management
- **integration-testing-expert**: Audio testing strategies, async testing, pytest frameworks, mock strategies

#### 🔍 Quality Assurance & Critics
- **code-architecture-reviewer**: Clean architecture, SOLID principles, design pattern analysis
- **performance-optimization-critic**: System optimization, profiling, resource utilization analysis
- **security-compliance-auditor**: OWASP standards, regulatory compliance, systematic security audits

#### 📋 Project Management & Coordination  
- **audio-project-planner**: Technology roadmaps, feature prioritization, audio product strategy
- **memory-manager**: Knowledge base curation, cross-agent coordination, context management

### Usage Guidelines
Sub-agents are automatically invoked based on trigger conditions in their descriptions. They provide expert analysis and recommendations without directly modifying source files, enabling safe parallel processing and comprehensive domain coverage.
