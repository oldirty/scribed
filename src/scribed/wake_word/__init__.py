...
class WakeWordEngine:
            ...
            for keyword in self.keywords:
                if isinstance(keyword, str) and keyword.endswith('.ppn'):
                    ...
                    if not Path(keyword).exists():
                        raise WakeWordDetectionError(f"Custom model file not found: {keyword}")
    ...
    def _initialize_audio(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Audio stream opened - Sample rate: {self.porcupine.sample_rate}Hz")
        ...
        try:
            while self._is_listening:
                ...
                if keyword_index >= 0:
                    ...
                    self.logger.info(f"Wake word detected: {keyword_name} (index: {keyword_index})")
...
class AsyncWakeWordEngine:
    ...
    async def start_listening(self, callback: Callable[[int, str], Any]) -> None:
        ...
        def queue_callback(keyword_index: int, keyword_name: str):
            try:
                ...
                asyncio.create_task(self._callback_queue.put((keyword_index, keyword_name)))
            except Exception as e:
    ...
    async def stop_listening(self) -> None:
        ...
        self._running = False
        await asyncio.get_event_loop().run_in_executor(
            None, self.engine.stop_listening
        )
