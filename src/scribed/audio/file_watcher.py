            ...
            if transcription_service and transcription_service.is_available():
                ...
                if result.status.value == "completed":
                    ...
                    content_lines = [
                        ...
                        f"# Processing time: {result.processing_time:.2f}s" if result.processing_time else "",
                    ...
                    if result.segments:
                        ...
                        for i, segment in enumerate(result.segments, 1):
                            start_time = f"{segment.start_time:.2f}s" if segment.start_time else "N/A"
                            end_time = f"{segment.end_time:.2f}s" if segment.end_time else "N/A"
                            content_lines.append(f"{i}. [{start_time} - {end_time}] {segment.text}")
                ...
                logger.warning("No transcription service available, creating placeholder")
                transcript_content = f"[Transcription placeholder for {file_path.name}]\n"
