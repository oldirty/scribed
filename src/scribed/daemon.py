...
from .realtime.transcription_service import RealTimeTranscriptionService, TranscriptionState
...
class ScribedDaemon:
    ...
    def _initialize_transcription_service(self) -> None:
        ...
        try:
            self.transcription_service = TranscriptionService(self.config.transcription.dict())
            if self.transcription_service.is_available():
                engine_info = self.transcription_service.get_engine_info()
                self.logger.info(f"Transcription service initialized: {engine_info['provider']}")
            else:
                self.logger.warning("Transcription service not available - check configuration")
    ...
    async def _start_realtime_service(self) -> None:
        ...
        try:
            ...
            # Set up callbacks
            self.realtime_service.set_transcription_callback(self._on_transcription_result)
            self.realtime_service.set_wake_word_callback(self._on_wake_word_detected)
            self.realtime_service.set_state_change_callback(self._on_realtime_state_change)
            
            # Start the service
            await self.realtime_service.start_service()
            
            self.logger.info("Real-time transcription service started with wake word detection")
    ...
    async def _on_transcription_result(self, result: Any, partial: bool) -> None:
        ...
        self.logger.info(f"{result_type.capitalize()} transcription result: {transcription}")
        ...
        if not partial:
            try:
                ...
                with open(output_file, 'w', encoding='utf-8') as f:
                    f.write(f"# Real-time transcription - {datetime.datetime.now().isoformat()}\n")
    ...
    async def _on_realtime_state_change(self, old_state: 'TranscriptionState', new_state: 'TranscriptionState') -> None:
        """Handle real-time service state changes."""
        self.logger.debug(f"Real-time service state changed: {old_state.value} -> {new_state.value}")
