...
class EnhancedWhisperEngine(TranscriptionEngine):
    ...
    def _load_model(self):
        ...
        raise RuntimeError(
            f"Failed to load any Whisper backend. Last error: {last_error}"
        )
    ...
    def _transcribe_faster(self, audio_file_path: str) -> dict:
        ...
        segments, info = self._model.transcribe(
            audio_file_path, language=self.language, word_timestamps=False
        )
        ...
        return {"text": full_text, "segments": text_segments, "language": info.language}
