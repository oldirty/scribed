...
class TranscriptionService:
    ...
    def _initialize_engine(self) -> None:
        ...
        try:
            ...
            if self._engine.is_available():
                self.logger.info(f"Transcription engine '{self.provider}' initialized successfully")
            else:
                self.logger.warning(f"Transcription engine '{self.provider}' is not available")
                
        except Exception as e:
            self.logger.error(f"Failed to initialize transcription engine '{self.provider}': {e}")
            self._engine = None
    ...
    async def transcribe_file(self, audio_file_path: Union[str, Path]) -> TranscriptionResult:
        ...
        if result.status == TranscriptionStatus.COMPLETED:
            self.logger.info(f"Transcription completed in {result.processing_time:.2f}s")
        else:
    ...
    def switch_provider(self, provider: str, config_override: Optional[dict] = None) -> bool:
        ...
        if self.is_available():
            self.logger.info(f"Successfully switched from '{old_provider}' to '{provider}'")
            return True
        else:
            self.logger.error(f"Failed to switch to '{provider}', reverting to '{old_provider}'")
